...
4. **Consolidate once, at the end.** After all subagents have returned (or failed/timed out — see Error Handling), the orchestrator merges every returned array into one combined list and Write the consolidated output to output/austin_turf_leads_<TIMESTAMP>.json, where <TIMESTAMP> is the UTC run time formatted YYYYMMDDTHHMMSSZ (e.g. output/austin_turf_leads_20260701T143000Z.json). Never write to a fixed filename that a prior run may have created; each run produces a new, uniquely named file so earlier results are preserved. This is the only file written.
